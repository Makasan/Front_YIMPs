...
def ScrimBorad ():
    ...
    def showMyRequest():
        ...
        clear = Label(myFrame).place(height=1000,width=1000)
        clearsort = Label(win,bg=back_ground).place(relx=0.5,rely=0.18,height=70,width=100)
        #myFrame.destroy()
        ...
        myPost_Label.place(x=250,y=160,width=300,height=45)
    ...
    def viewMyRequest (id_post):
        ...
        label_TeamName = Label(myRequest,bg='black').grid(row=coutRow,column=coutCol,ipadx=20,ipady=10,padx=20,pady=10)
        coutCol += 1
        ...
        if teamDataPost != [] :
            ...
            for i in range(len(teamDataPost)):
                label_TeamName = Label(myFrame,bg='black').grid(row=coutRow+i,column=coutCol,ipadx=20,ipady=10,padx=10)
                coutCol += 1
    ...
    search_button.place(relx=0.33,rely=0.23)
    if user["teamID"] != '':
...
def Profile():
    respone = requests.get("http://34.124.169.53:8000/api/login",data=id_user, headers={'Content-Type': 'application/x-www-form-urlencoded'})
    user = dict(respone.json()) 
    clear = Label(win,bg="pink").place(x=0,y=0,width=window_width,height=window_height) 
    def updateProfile():
...
def HomePage():
    ...
    if fivemath['nextFivePost'] != []:    
        match_Incom_Label = Label(win, text=((fivemath['nextFivePost'][0])['enemyTeamName'])+"  VS  "+((fivemath['nextFivePost'][0])['teamName']),fg='red' ,font=('Arial',15),bg=back_ground)
